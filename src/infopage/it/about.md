# Informazioni su Atomic Search

Atomic Search è un [metamotore di ricerca] che aggrega i risultati di altri motori di
ricerca.  {{link('motori di ricerca', 'preferences')}} e non memorizza
informazioni sui suoi utenti.

Il progetto Atomic Search è guidato da una comunità aperta, unisciti a noi su Matrix
se hai domande o vuoi semplicemente chiacchierare.  Se avete domande o volete
semplicemente parlare di Atomic Search a [#searxng:matrix.org].

Migliorate Atomic Search.

- Potete migliorare le traduzioni di Atomic Search su [Weblate], oppure...
- Seguire lo sviluppo, inviare contributi e segnalare problemi a [sorgenti di
  Atomic Search].
- Per ulteriori informazioni, visitate la documentazione del progetto Atomic Search su
  [Atomic Search docs].

## Perché usarlo?

- Atomic Search non offre risultati personalizzati come Google, ma non genera un
  profilo dell'utente.
- Atomic Search non si preoccupa di ciò che cercate, non condivide mai nulla con terze
  parti e non può essere usato per compromettere l'utente.
- Atomic Search è un software libero, il codice è aperto al 100% e tutti sono invitati
  a migliorarlo.

Se avete a cuore la privacy, se volete essere un utente consapevole o se credete
nella libertà digitale, fate di Atomic Search il vostro motore di ricerca predefinito
o eseguitelo sul vostro server!

## Come si imposta come motore di ricerca predefinito?

Atomic Search supporta [OpenSearch].  Per ulteriori informazioni sulla modifica del
...
- rimuovere il duplicato (nome predefinito: Atomic Search) o
- contattare il proprietario per assegnare all'istanza un nome diverso da quello
  predefinito.

## Come funziona?

Atomic Search è un fork del ben noto [searx] [metamotore di ricerca] che è stato
ispirato dal [progetto Seeks].  Fornisce una privacy di base mescolando le
ricerche su altre piattaforme senza memorizzare i dati di ricerca.  Atomic Search può
...
Atomic Search apprezza la vostra preoccupazione per i log, quindi prendete il codice
dal file [sorgenti di Atomic Search] ed eseguitelo voi stessi!
...
[sorgenti di Atomic Search]: {{GIT_URL}}
